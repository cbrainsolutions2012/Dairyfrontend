...
function Expenses() {
  ...
  // Fetch expense report data from API
  const fetchExpenseData = async () => {
    if (!formData.sdate || !formData.edate) return;
    setLoading(true);
    try {
      const res = await fetch(
        `https://api.mytemplesoftware.in/api/reports/expense?startDate=${formData.sdate}&endDate=${formData.edate}&detailed=true`,
        {
          headers: {
  ...
  const handleExportToExcel = () => {
    ...
    const excelData = expenseData.map((item, idx) => ({
      अनुक्रमांक: idx + 1,
    ...
    utils.book_append_sheet(wb, ws, 'खर्च अहवाल');
    writeFile(wb, `खर्च_अहवाल_${formData.sdate}_to_${formData.edate}.xlsx`);
  };
      ...
      <Col sm={12}>
        <Card>
          <Card.Header>
            <Row>
              <Col md={6}>
                <Card.Title as="h4">खर्च अहवाल</Card.Title>
              </Col>
