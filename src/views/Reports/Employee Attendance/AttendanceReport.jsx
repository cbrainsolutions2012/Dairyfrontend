...
function AttendanceReport() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(formErrors).length === 0) {
      ...
      try {
        ...
        const response = await axios.get(`https://api.mytemplesoftware.in/api/employee-reports/attendance?${params}`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`
          }
        });
        ...
        setError(error.response?.data?.message || 'Error fetching attendance report');
        setReportData(null);
      } finally {
        setLoading(false);
      }
  ...
  const handleExportToPDF = async () => {
    ...
    try {
      ...
      // Create canvas from the table element with simple settings
      ...
      pdf.setFontSize(16);
      pdf.text('Employee Attendance Report', 140, 10, { align: 'center' });
      ...
      const currentDate = new Date().toISOString().split('T')[0];
      pdf.save(`Attendance_Report_${currentDate}.pdf`);
    } catch (error) {
        ...
        <Col sm={12} className="mt-4">
          <Card>
            <Card.Header>
              <Row>
                <Col md={6}>
                  <Card.Title as="h4">{reportData?.reportTitle || 'कर्मचारी उपस्थिती अहवाल'}</Card.Title>
                  {reportData && (
                    <small className="text-muted">
                      तारीख: {new Date(reportData.dateRange.startDate).toLocaleDateString('en-IN')} ते{' '}
                      ...
                      {filteredRecords.length > 0 ? (
                        filteredRecords.map((record, index) => (
                          <tr key={index}>
                            ...
                            <td>
                              <span className={`badge bg-${getStatusBadge(record.Status)}`}>{record.Status}</span>
                            </td>
                          </tr>
                        ))
                      ) : (
