...
function BuyersPayment() {
  ...
  // Add this function to fetch buyer details when buyer is selected
  const fetchBuyerDetails = async (buyerId) => {
    ...
    try {
      const res = await axios.get(`${API_URL}/api/buyers/${buyerId}?includeTransactions=true`, {
        headers: getAuthHeaders()
      });

      if (res.data.success) {
        setSelectedBuyerDetails(res.data.data.buyer);
      }
    } catch (error) {
  ...
  const handleChange = (e) => {
    ...
    // Fetch buyer details when buyer is selected
    if (name === 'buyerId' && value) {
      fetchBuyerDetails(value);
    } else if (name === 'buyerId' && !value) {
      setSelectedBuyerDetails(null);
    }
  ...
  const handleSearchChange = (e) => {
    ...
    if (value === '') {
      ...
      const filteredData = allPaymentsData.filter(
        (p) =>
          (p.BuyerName || '').toLowerCase().includes(value.toLowerCase()) ||
  ...
  const fetchPaymentsData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API_URL}/api/buyerpayments`, { headers: getAuthHeaders() });
      if (res.data.success) {
  ...
  const fetchBuyersData = useCallback(async () => {
    try {
      const res = await axios.get(`${API_URL}/api/buyers`, { headers: getAuthHeaders() });
      if (res.data.success) {
  ...
  useEffect(() => {
    fetchPaymentsData();
    fetchBuyersData();
  }, [fetchPaymentsData, fetchBuyersData]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put(`${API_URL}/api/buyerpayments/${editingPaymentId}`, formData, { headers: getAuthHeaders() });
        alert('Payment updated successfully!');
      } else {
        await axios.post(`${API_URL}/api/buyerpayments`, formData, { headers: getAuthHeaders() });
  ...
  const confirmDelete = (id, buyerName, date) => {
    if (window.confirm(`Are you sure you want to delete this payment for "${buyerName}" on ${date}?`)) {
      handleDelete(id);
    }
  };
  ...
  const handleDelete = async (id) => {
    setLoading(true);
    try {
      await axios.delete(`${API_URL}/api/buyerpayments/${id}`, { headers: getAuthHeaders() });
  ...
  const handleExportToExcel = async () => {
    ...
    try {
      ...
      utils.book_append_sheet(wb, ws, 'Buyer Payments');
      const filename = `BuyerPayments_${new Date().toISOString().slice(0, 19).replace(/[:-]/g, '')}.xlsx`;
  ...
  const handleExportToPDF = async () => {
    ...
    try {
      ...
      pdf.text('Buyer Payments Report', 105, 20, { align: 'center' });
      const headers = [['Sr.', 'Buyer Name', 'Date', 'Amount (₹)', 'Type', 'Details']];
      ...
      const filename = `BuyerPayments_${new Date().toISOString().slice(0, 19).replace(/[:-]/g, '')}.pdf`;
      pdf.save(filename);
        ...
        <Card>
          <Card.Header>
            <Card.Title as="h4" style={{ display: 'flex', justifyContent: 'center' }}>
              {isEditing ? 'Edit Buyer Payment' : 'Add New Buyer Payment'}
            </Card.Title>
          </Card.Header>
              ...
              {selectedBuyerDetails && (
                <div className="form-row">
                  <div className="form-group" style={{ width: '100%' }}>
                    ...
                    >
                      <h6 style={{ marginBottom: '10px', color: '#495057' }}>{selectedBuyerDetails.FullName} - Payment Summary</h6>
                      <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap' }}>
                        <div style={{ minWidth: '120px', marginBottom: '5px' }}>
                          <small style={{ color: '#6c757d' }}>Total Business:</small>
                          <div style={{ fontWeight: 'bold' }}>₹{selectedBuyerDetails.milkTransactions?.totalAmount || 0}</div>
                        </div>
                        <div style={{ minWidth: '120px', marginBottom: '5px' }}>
                          <small style={{ color: '#6c757d' }}>Total Received:</small>
                          <div style={{ fontWeight: 'bold', color: 'green' }}>₹{selectedBuyerDetails.payments?.totalPaid || 0}</div>
                        </div>
                        <div style={{ minWidth: '120px', marginBottom: '5px' }}>
                          <small style={{ color: '#6c757d' }}>Outstanding:</small>
                          <div
                            style={{
                              fontWeight: 'bold',
                              color: selectedBuyerDetails.outstandingAmount > 0 ? 'red' : 'green'
                            }}
                          >
                            ₹{selectedBuyerDetails.outstandingAmount || 0}
                          </div>
                        </div>
                        {selectedBuyerDetails.outstandingAmount > 0 && (
                          <div style={{ minWidth: '120px', marginBottom: '5px' }}>
                            <Button
                              ...
                              onClick={() =>
                                setFormData({
                                  ...formData,
                                  paymentAmount: selectedBuyerDetails.outstandingAmount
                                })
                              }
                              style={{ fontSize: '12px' }}
                            >
          ...
          <Card.Header>
            <Row>
              ...
              >
                <Form.Control
                  type="text"
                  placeholder="Search by buyer, type, amount..."
                  value={searchTerm}
                          ...
                          <tr key={p.Id}>
                            ...
                            <td>
                              ...
                              <Button
                                variant="danger"
                                size="sm"
                                onClick={() => confirmDelete(p.Id, p.BuyerName, new Date(p.PaymentDate).toLocaleDateString('en-IN'))}
                                disabled={loading}
                  ...
                  {currentData.map((p, index) => (
                    <div key={p.Id} className="mobile-card">
                      <div className="mobile-card-header">
                        <div className="mobile-card-title">{p.BuyerName}</div>
